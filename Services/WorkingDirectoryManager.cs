...
namespace QB_TimeWarp.Services
{
    ...
    public class WorkingDirectoryManager
    {
        ...
        private void CopySourceFile(bool forceRefresh)
        {
            ...
            // Also copy associated QuickBooks files (.qbw.nd, .qbw.tlg, etc.)
            CopyAssociatedFiles(
                Path.GetDirectoryName(OriginalSourceFilePath)!,
                _workingConfig.SourcePath,
                Path.GetFileNameWithoutExtension(OriginalSourceFilePath));
        }
        ...
        private void CopyTargetFile(bool forceRefresh)
        {
            ...
            // Also copy associated QuickBooks files
            CopyAssociatedFiles(
                Path.GetDirectoryName(OriginalTargetFilePath)!,
                _workingConfig.TargetPath,
                Path.GetFileNameWithoutExtension(OriginalTargetFilePath));
        }
        ...
        /// <summary>
        /// Copies associated QuickBooks files (.nd, .tlg, .dsn, etc.) that may be needed.
        /// These are NOT critical but can help QuickBooks open the file.
        /// </summary>
        private static void CopyAssociatedFiles(string sourceDir, string destDir, string baseName)
        {
            var associatedExtensions = new[] { ".qbw.nd", ".qbw.tlg", ".qbw.dsn" };
            var copied = 0;

            foreach (var ext in associatedExtensions)
            {
                var sourceFile = Path.Combine(sourceDir, baseName + ext);
                if (File.Exists(sourceFile))
                {
                    var destFile = Path.Combine(destDir, baseName + ext);
                    try
                    {
                    ...
                    {
                        Log.Debug("Could not copy associated file {File}: {Message}",
            ...
            if (copied > 0)
                Log.Information("    Also copied {Count} associated QB files", copied);
        }
